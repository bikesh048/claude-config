...
Single entry point for all development work. Handles ticket creation/reading, planning with interview, phase-by-phase execution with incremental commits, PR creation, and ticket updates.
...
/deliver "fix the broken image upload"   # Adhoc — no ticket
```
...
Parse the arguments to determine the flow:

| Input | Flow | Example |
|-------|------|---------|
| `OP#NNNN` or just a number | **Existing ticket** | `/deliver OP#1790` or `/deliver 1790` |
| `<parent_id> "description"` | **New ticket** | `/deliver 1322 "build envctl"` |
| `"description"` only | **Adhoc** (no ticket) | `/deliver "fix broken upload"` |

## Flow 1: Existing Ticket

```
op-read → plan + interview → branch → execute → create-pr → op-update
```

### Step 1: Read Ticket

Run `/op-read ${ticket_id}` to fetch:
...
```
plan + interview → op-create (from plan) → branch → execute → create-pr → op-update
```
...
Run `/op-create` automatically using the plan output:
- **Parent**: from arguments (e.g. `1322`)
...
- **Quick Snippets**: branch name is the session contract
...
- **GoCD push fails**: Keep local changes, print retry command
- **Phase validation fails after 3 attempts**: Commit what works, note failure in commit message, continue
...
6. **Single source of truth** — delegate to `/create-pr`, `/op-create`, `/op-read`, `/op-update`
